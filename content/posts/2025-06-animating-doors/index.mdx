...
description: 'Animating doors in a 3D dungeon'
...
React and animation can present challenges due to React's core principle: a function transforms data into a view \(Function(Data) = View\). This paradigm works well for static UIs. However, smooth animations often involve directly manipulating elements without altering the underlying data.
...
Jotai uses an atomic approach to state management. Each atom is contains a piece of data.

You can also create derived atoms - read-only atoms, write-only atoms, and a mix of the two.


This is a diagram of the atoms used
...
moveToRoom is provided with three actions:
...
all of the behaviour is done within the moveToRoomAtom

1. open the door and move the camera to the door

- the current room id is fetched from the dungeonCurrentRoomAtom

- the door (model) is fetched from the dungeonCurrentRoomAtom

- the door is opened. two things happen at once here:
  - moveCameraAction is called - this moves the camera to the door location
  - the doorAction is called - this begins the door animation

2. update the door status

- once that is done, the state of the door is updated in the dungeon model (dungeonAtom)

- the destination room is fetched with a combination of the current room and the door

3. enter the target room

- the camera is moved to the center of the target room
- the state is updated so that this room now becomes the current

4. close the door

- doorAction is called, which animates the door closing
- the door state is updated in the model

5. unmount the old room

- unmountRoomAction is called - this unmounts all the components in the room

- the visible state of the previous room and doors is cleared with setDungeonVisibleAtom
...
- dungeonCurrentRoomAtom - stores the id of the current room
- dungeonAtom - stores the dungeon state object
- setDungeonVisibleAtom - setter atom - updates:
    - dungeonVisibleRoomsAtom - an array of Room ids which are visible
    - dungeonVisibleDoorsAtom - an array of Door ids which are setDungeonVisibleAtom

- hasVisitedRoomAtom - a getter atom which returns whether a given roomId has been hasVisitedRoomAtom
- dungeonVisitedRoomsAtom - an array of room ids which have been visited
